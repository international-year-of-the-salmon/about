...
published_data <- read_sheet("https://docs.google.com/spreadsheets/d/1SoeRT_eytuMS3-Ap5q6UPzcwBK2HKiqzsTbWLmxZcBA/edit#gid=1459056304", sheet = "Published") %>% 
  mutate(Year = str_extract_all(`Dataset Title`,"\\(?[0-9,.]+\\)?")[[1]],
         Vessel = if_else(Year == 2019, "Professor Kaganovsky", "Pacific Legacy NO 1")) %>% 
...
reactable(published_data, 
    ...
    function(cellInfo) {
      // Render as a link
      const url = cellInfo.row['Digital Object Identifier'] + '_' + cellInfo.value
